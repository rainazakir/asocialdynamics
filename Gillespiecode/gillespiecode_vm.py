...
def gillespieStep(state, N, gammas, alphas, rhos, sigmas, vectorsOfChange, timeLeft):
    ...
    numOptions = int((len(state)) / 2)
    for i in range(0,
                   numOptions):  # PAY ATTENTION THAT i IS NOT THE CORRECT state[i] BUT IT MUST BE state[i+1], WHILE WORKS FOR gammas[i], alphas[i], rhos[i]
        # Discovery
        ...
        if ((state[0] + state[1])) <= 1:
            ...
            probabilitiesOfChange.append(
                state[0] * state[1] * ((1 - noisevalue) / (qualities[i] * t_d)) / ((state[0] + state[1]) - 1))
        ...
        if ((state[0] + state[1])) <= 1:
            ...
            probabilitiesOfChange.append(
                state[i] * state[i] * ((1 - noisevalue) / (qualities[i] * t_d)) / ((state[0] + state[1]) - 1))

        ############# Go to Dissemination after Exploration #############
        probabilitiesOfChange.append(state[i + 2] * (1 / t_e))  # AE -> AD and BE -> BD

        ############# Noisy Switch from Kilogrid #############
        probabilitiesOfChange.append(state[i] * ((noisevalue * nr1[i]) / (qualities[i] * t_d)))  # AD->BE and BD->AE
        probabilitiesOfChange.append(state[i] * ((noisevalue * nr2[i]) / (qualities[i] * t_d)))  # AD->AE and BD->BE
...
def runGillespie(state, T, N, gammas, alphas, rhos, sigmas, rnd_seed, finalStateFile, temporalEvolution, plot_evo,
    ...
    for i in [1, 2]:
        if (i == 1):  # pop A
            # [AD BD AE BE]  # the important thing is that sum is ZERO
            negative = [-1, 0, 0, 1]  # for cross inhibition
            # plus = [-1, 0, 0, 1, 0]  # for recruitment
            sameplus = [-1, 0, 1, 0]  # for no-crossinhibition in voting
            expplorationend = [1, 0, -1, 0]
            noiseopposite = [-1, 0, 0, 1]
            noisesame = [-1, 0, 1, 0]
        ...
        if (i == 2):  # pop B
            ...
            # plus = [-1, 0, 0, 0, 1]
            sameplus = [0, -1, 0, 1]
            expplorationend = [0, 1, 0, -1]
            noiseopposite = [0, -1, 1, 0]
            noisesame = [0, -1, 0, 1]
    ...
    while t < T:
        ...
        spd[int(previous_state[0]) + int(previous_state[2])][
            int(previous_state[1]) + int(previous_state[3])] += time_step
        ...
        if (quorum > 0):  # 0.79>0 true
            ...
            for i in np.arange(1, len(state) - 2):
                ...
                if ((state[i - 1] + state[i + 1]) >= N * quorum):
    ...
    for valueA in qa:
        ...
        for valueB in valuesB:
            ...
            for i in range(0, repetitions):
                ...
                # state = [round((N - theexplorationstate) / 2), round((N - theexplorationstate) / 2),
                # round(theexplorationstate / 2), round(theexplorationstate / 2)]
                #state = [2, 26, 8, 64]
                print(state)
                rnd_seed = np.random.randint(4325344)
                temporalEvolution = 'none'
                temporalEvolution = 'popevo_vm0.05_' + str(state) + str(t_u) + "td" + str(t_d) + "te" + str(
                    t_e) + "_" + str(nr2[0]) + "noise_" + "500ksteps_" + str(repetitions) + "runs_" + str(
                    N) + "agents_" + str(qualities) + str(quorum) + "/evoN" + str(i) + "_" + str(rnd_seed) + '.txt'
                # rnd_seed = np.random.randint(4325344)
                # rnd_seed = 43431
                print(rnd_seed)
                # S = N
                ...
                # state = [round((N - theexplorationstate) / 2), round((N - theexplorationstate) / 2),
                #         round(theexplorationstate / 2), round(theexplorationstate / 2)]
                # state = [N/4,N/4,N/4,N/4]
                # print(state)
                # state = U, AD, BD, AE, BE
                print(N, "Finding CDCI:", "(", state, ")  QRatio: ", qualities, nr1, nr2)
                # state = [N-Z] + [0]*n + [Z/n]
...
outputname = "vm0.05_outputoftu" + str(state) + str(t_u) + "td" + str(t_d) + "te" + str(t_e) + "_" + str(
    nr2[0]) + "noise_" + "500ksteps_" + str(repetitions) + "runs_" + str(N) + "agents_" + str(qualities) + ".txt"
