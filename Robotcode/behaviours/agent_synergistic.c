...
double noise = 0.10; // SET THIS TO -1 FOR NO NOISE
int how_many_op = 2; //set the number of opinion in Kilogrid- max is 4 for now
double avg_exploration_time = 3300; //time to be in exploration state
double avg_uncommitted_time = 0000.0; // time dormant for uncommitted agents
double dissemparam = 1300.0; //time to stay in dissemination
/*-----------------------------------------------------------------------------------------------*/
int prev_option;
...
//tracking of quality from kilgrid
...
//initialize eexperiment with robots in exploration state
...
/* of  its opinion. The function is inverse lambda expon.       */
/*-----------------------------------------------------------------------------------------------*/
...
void setup(){
	...
	// set initial opinion of robots
	////choose my opinion based on odd or even kilouid
	if (how_many_op == 3) {
        if (kilo_uid % 3 == 0) {
            currentopinion = 1;
    ...
    message.data[2] = kilo_uid;
    message.crc = message_crc(&message);
    timer =  ran_expo(1.0/avg_exploration_time); //set time to be in exploration state
