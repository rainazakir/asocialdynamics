                                           ...
                                           # print(((int(line.split()[2]) + int(line.split()[4]))),
                                             #    ((int(line.split()[3]) + int(line.split()[5]))))
                                               if(line.split()[7]==quorumtocheckfor):
                                                   checkQuorum = True
                                               if (checkQuorum and line.split()[7]=='end'):
                                            ...
                                            if(line.split()[6]==quorumtocheckfor):
                                                checkQuorum = True
                                                    ######### print(line)
                                            if(checkQuorum and line.split()[6]=='end'):
                                                        timestepquorumrreached = timestepquorumrreached + float(line.split()[0])
                                                        timestepquorumrreachedconverged = timestepquorumrreachedconverged + float(line.split()[0])
                                                        reachedquorum +=1
...
data = pd.DataFrame({'h': xaxis, 'k': yaxis, 'br': ds_accuracy})
...
plt.savefig('/Volumes/My_Passport/argosim2/Runs/journal/imageshmap/GABS_norm_hmap_'+str(typemod)+'_coh_q'+str(qr)+'_'+str(quorumtocheckfor)+'.pdf', dpi=300, bbox_inches='tight')
