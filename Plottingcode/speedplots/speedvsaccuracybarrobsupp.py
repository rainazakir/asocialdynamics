...
opdir='/Volumes/My_Passport/280423_supp/selected1n/'
...
filesnum  = 0
findnoise = '0.05noise' #change this
findqr = '[1.0, 1]0.95' #change this
findstdv = 'stdDv-1'
...
for subdir, dirs, files in sorted(os.walk(opdir)):
    for name in sorted(dirs):
       ...
       # print(name.split('_')[3].split('n')[0])
        #print(name.split('_')[2].split(']')[1].split('td')[0])
        typemod = name.split('_')[1]
        #print(typemod)
        quorum = float(name.split('_')[7].split(']')[1])
        noise = float(name.split('_')[3].split('n')[0])
        N=100
        tu = float(name.split('_')[2].split(']')[1].split('td')[0])
        print(tu,quorum,N,quorum*N,typemod, noise)
        #qr = float(name.split('_')[7])
       # print(noise,qr)
        #quorum = 0.8 * (norm*100)
        #print(name.split('_'))
        #name.split('_')[3] == str(findnoise) and
        if(name.split('_')[3] == str(findnoise) and name.split('_')[7] == str(findqr)):
...
df = pd.DataFrame({"range": typemodall,"tu": typemodallN, "time":dectime,"error":stddec})
fig, ax = plt.subplots()

#ax = df.pivot("tu", "range", "time").plot(kind='bar',color=['tab:blue','tab:pink'], rot=0,legend=False)

ax = df.pivot("tu", "range", "time")
yerr = df.pivot("tu", "range", "error")
ax.plot(kind='bar',colormap='PiYG', rot=0,yerr=yerr,error_kw=dict(lw=5, capsize=20, capthick=4),legend=False)
#plt.show()
ax = plt.gca()
...
plt.savefig('/Volumes/My_Passport/argosim2/Runs/journal/images/jan3_changetu_n0.05_speed_qr1.0_1.pdf', dpi=300, bbox_inches='tight')
